...
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
USBD_HandleTypeDef USBD_Device;             /* USB DeviceÂ§ÑÁêÜÁªìÊûÑÔø????????? */
extern volatile uint8_t g_usb_state_reg;    /* USBÁä∂ÔøΩ? */
extern volatile uint8_t g_device_state;     /* USBËøûÊé• ÊÉÖÂÜµ */
...
int main(void)
{
  ...
  delay_init(72);                         /* ÔøΩÔøΩ ±ÔøΩÔøΩ ºÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ∆µ72 MHz */
...
//  stmencrypt_init();
...
#if 0
  usbd_port_config(0);    /* USBÂÖàÊñ≠ÂºÄ */
  HAL_Delay(500);
  usbd_port_config(1);    /* USBÂÜçÔøΩ?ÔøΩËøûÔø????????? */
  HAL_Delay(500);
  USBD_Init(&USBD_Device, &MSC_Desc, 0);                              /* ÂàùÔøΩ?ÔøΩÂåñUSB */
  USBD_RegisterClass(&USBD_Device, USBD_MSC_CLASS);                   /* Ê∑ªÂä†Ôø????????? */
  USBD_MSC_RegisterStorage(&USBD_Device, &USBD_DISK_fops);            /* ‰∏∫MSCÁ±ªÊ∑ªÂä†ÂõûË∞ÉÂáΩÔø????????? */
  USBD_Start(&USBD_Device);                                           /* ÂºÄÂêØUSB */
   while(1);
  HAL_Delay(5000);
#endif
  ...
  text_show_font(200, 100, "µÁ◊”≥∆", 12, 0, RED);
  text_show_font(200, 112, "µÁ◊”≥∆", 16, 0, RED);
  text_show_font(200, 128, "µÁ◊”≥∆", 24, 0, RED);
  text_show_font(200, 152, "µÁ◊”≥∆", 32, 0, RED);
